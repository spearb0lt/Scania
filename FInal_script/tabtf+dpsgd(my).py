...
for epoch in range(1, NUM_EPOCHS+1):
    ...
    #my variables
    clip_bound= MAX_GRAD_NORM
    spec_k = 2  # Set as per your requirement, or None for no filtering
    sigma=0.1
    ...
    print(f"Train MSE: {train_mse:.4f} | Val MSE: {val_mse:.4f} | σ={sigma:.3f} | C={MAX_GRAD_NORM:.3f}\nTime: {h}hrs. {m}min. {s}sec.")
    ...
    # early stop
    if val_mse < best_val:
        best_val = val_mse; no_improve = 0; torch.save(model.state_dict(), 'best_dp_model_adaptive_largeralpha.pt')
...
# Train MSE: 267.9540 | Val MSE: 267.7559 | σ=0.100 | C=1.000
# Time: 0hrs. 2min. 45sec.
# Model saved with val loss: 267.75592912780155

# Epoch 02 →

# Train MSE: 217.3054 | Val MSE: 232.7041 | σ=0.100 | C=1.000
# Time: 0hrs. 2min. 47sec.
# Model saved with val loss: 232.70412004530476
# c:\Users\ASUS\Downloads\GUEST\custom_dp.py:280: UserWarning: Optimal oconsider expanding the range of alphas to get a tighter privacy bound.
#   warnings.warn(
# DP-SGD with
#         sampling rate = 4.88%,
#         noise_multiplier = 0.1,
#         iterated over 42 steps,
# satisfies differential privacy with
#         epsilon = 1.05e+03,
#         delta = 6.3374685224095e-05.
# The optimal alpha is 1.1.
# The privacy estimate is likely to be improved by expanding the set of alpha orders.
# Final (ε,δ)=(1051.14,6.3374685224095e-05), optimal α=1.1
...
# code is a correct implementation of spectral DP at the batch level.
# For production or strong privacy guarantees, consider per-sample spectral DP and a privacy accountant tailored for spectral mechanisms.
# For research and experimentation, your approach is valid and implements the main ideas of spectral DP.