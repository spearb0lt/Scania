...
class RULClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # DP params (set defaults if not provided)
        sigma = config.get("dp_sigma", 0.1)
        clip_bound = config.get("dp_clip_bound", 1.0)
        spec_k = config.get("dp_spec_k", 2)
        ...
        for _ in range(epochs):
            for x_cat, x_ts, y in train_loader:
                ...
                loss.backward()
                if use_dp:
                    spectral_dp_gradient_update(self.model, sigma, clip_bound, spec_k)
                optimizer.step()
        # Evaluate locally
